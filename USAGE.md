...
- **智能任务管理**: 自动记录和恢复任务状态，避免重复提交
- **自动状态监控**: 每20秒自动查询RunningHub任务状态
- **任务持久化**: 任务信息保存到JSON文件，支持断点续传
- **视频自动组合**: 所有视频生成完成后自动组合成完整视频
- **避免重复提交**: 已提交的任务不会重复提交，直接轮询等待结果

#### 任务状态文件

程序会在`input/`文件夹下创建与输入PPT文件同名的`.json`文件来保存任务状态和配置信息，包含：
- 项目信息
- 任务总数
- 已完成任务数
- 失败任务数
- 每个任务的详细信息（页码、内容、模式、状态、任务ID等）
- RunningHub API配置
- 数字人配置信息
- 工作室效果图路径

例如：如果输入文件是`演示文稿.pptx`，则任务状态文件为`input/演示文稿.json`

#### 运行流程

1. **读取PPT文件**: 从`input/`目录读取PPTX文件
2. **提取备注**: 提取每页的备注内容作为数字人播报文本
3. **提交任务**: 向RunningHub API提交数字人视频生成任务
4. **监控状态**: 每20秒查询一次任务状态
5. **等待完成**: 等待当前任务完成后提交下一个任务
6. **组合视频**: 所有任务完成后自动组合成完整视频

#### 断点续传

如果程序中断，只需重新运行`python main.py`，程序会自动：
- 读取对应的状态文件中的任务状态
- 继续处理未完成的任务
- 不会重复提交已提交的任务
- 完成后自动组合所有视频
